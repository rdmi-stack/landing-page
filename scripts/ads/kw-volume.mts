/**
 * Ad-hoc keyword search-volume lookup via the Google Ads Keyword Planner API.
 * Pulls India search volume + competition + top-of-page CPC for an arbitrary seed.
 *
 *   node scripts/ads/kw-volume.mts "beard growth oil"
 */
...
if (!seed) throw new Error('Pass a keyword, e.g. node scripts/ads/kw-volume.mts "beard growth oil"');
...
const exact = rows.find((r) => r.text.toLowerCase() === seed.toLowerCase());

console.log(`\nSeed: "${seed}"   (India · Google Search · avg monthly searches)\n`);
if (exact) {
  const cpc = exact.hi ? `₹${exact.lo.toFixed(0)}–${exact.hi.toFixed(0)}` : "—";
  console.log(`  EXACT MATCH → ${exact.vol.toLocaleString()} searches/mo · comp ${compLabel(exact.comp)} · CPC ${cpc}\n`);
} else {
  console.log(`  (no exact-match row returned; closest ideas below)\n`);
}
...
for (const r of rows.slice(0, 25)) {
  ...
  console.log(`  ${String(r.vol).padStart(7)}  ${compLabel(r.comp).padEnd(5)} ${cpc.padStart(14)}      ${r.text}`);
}
console.log("  " + "─".repeat(74));
console.log(`  ${rows.length} related ideas returned · top 25 shown\n`);
